...
class Config():
    ...
    cnn_f = [16,128,16,512]  # [cov1输出特征维度，cov2输出特征维度，cov3输出特征维度，全连接层输出维度]

    max_captcha = 4  #验证码最大长度
    char_set_len = 37  #字符集长度

    lr = 0.003
    batch_size = 25  # 每批训练大小
...
class Model():
    ...
    def train(self, get_next_batch, model_path, train_set, test_set):
        with self.session as sess:
            while True:
                batch_x, batch_y = get_next_batch(self.config.batch_size, train_set,False)
                ...
                if self.global_step.eval() % self.config.save_every_n == 0:
                    batch_x_test, batch_y_test = get_next_batch(100, test_set, True)
